async function Module(moduleArg = {}) {
	...
	function findWasmBinary() {
		if (Module['locateFile']) {
			return locateFile('kakadujs.wasm');
		}
		return new URL('kakadujs.wasm', import.meta.url).href;
	}
	...
	var ___syscall_poll = (fds, nfds, timeout) => {};
	...
	var whenDependentTypesAreResolved = (myTypes, dependentTypes, getTypeConverters) => {
		...
		var registered = 0;
		for (const [i, dt] of dependentTypes.entries()) {
			if (Object.hasOwn(registeredTypes, dt)) {
				typeConverters[i] = registeredTypes[dt];
			} else {
				unregisteredTypes.push(dt);
				if (!Object.hasOwn(awaitingDependencies, dt)) {
	...
	var init_ClassHandle = () => {
		const proto = ClassHandle.prototype;
		Object.assign(proto, {
			isAliasOf(other) {
	...
	var ensureOverloadTable = (proto, methodName, humanName) => {
		if (undefined === proto[methodName].overloadTable) {
			...
			proto[methodName] = function (...args) {
				if (!Object.hasOwn(proto[methodName].overloadTable, args.length)) {
					throwBindingError(
	...
	var exposePublicSymbol = (name, value, numArguments) => {
		if (Object.hasOwn(Module, name)) {
			...
			if (Object.hasOwn(Module[name].overloadTable, numArguments)) {
				throwBindingError(
	...
	function craftInvokerFunction(
		...
		const invokerFactory = createJsInvoker(argTypes, isClassMethodFunc, returns, isAsync);
